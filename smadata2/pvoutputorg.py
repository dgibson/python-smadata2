...
#        func = (lambda x : string.join(map((lambda y : [time.strftime("%Y%m%d",time.localtime(y)),time.strftime("%H:%M",time.localtime(y))]),x),','))
#        productiondata = string.join(map(func,batch),';')
	...
	for prodinfo in batch:
        ...
        filehandle = self.make_request(sid,"/service/r2/addbatchstatus.jsp",data)
...
#            'h': 1,
    ...
    def getstatus(self,sid,date,timefrom,timeto):
        ...
        filehandle = self.make_request(sid,'/service/r2/getstatus.jsp',request)
        ...
        for output in outputs:
            ...
            ret.append([outputentries[0],outputentries[1],int(outputentries[2])])
        return ret
    ...
    ## returns the number of days ago you can set values using the batchstatus script
    # @fixme rename me
